...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Cambiar contraseña — NO redirigimos
    if ($accion === "password") {
...
<head>
    ...
    <style>
        ...
        /* ---------- Avatar responsive ---------- */
        .avatar { width: 40px; height: 40px; border-radius: 50%; overflow: hidden; display: inline-block; flex: 0 0 auto; box-shadow: 0 2px 6px rgba(0,0,0,0.25); }
        .avatar > img { width: 100%; height: 100%; object-fit: cover; object-position: center; display: block; }
        .avatar--lg { width: var(--avatar-size, 100px); height: var(--avatar-size, 100px); border: 2px rgba(255, 10, 222, 0.82); box-shadow: 0 0 10px rgba(206, 2, 162, 0.64); animation: pulse 2s infinite; }
        @media (max-width: 575.98px) { .avatar--lg { --avatar-size: 84px; } }
        @media (min-width: 576px) and (max-width: 991.98px) { .avatar--lg { --avatar-size: 96px; } }
        @media (min-width: 992px) { .avatar--lg { --avatar-size: 112px; } }
        @keyframes pulse { 0%,100%{ box-shadow: 0 0 10px rgba(255,10,222,0.82);} 50%{ box-shadow: 0 0 25px rgba(245,12,194,0.84);} }

        /* ---------- Card ---------- */
        .card { background: linear-gradient(145deg, #141414, #1f1f1f); border: 1px solid rgba(0, 255, 255, 0.2); border-radius: 15px; transition: all .3s ease-in-out; }
        .card:hover { border-color: rgba(235, 9, 186, 0.88); box-shadow: 0 0 15px rgba(206, 2, 162, 0.64); }

        /* ---------- Botones ---------- */
        .btn-outline-light { border: 1px solid rgba(252, 6, 198, 0.86); color: rgba(238, 32, 193, 0.86); transition: all .3s; }
        .btn-outline-light:hover { background-color: rgba(219, 13, 174, 0.83); color: #111; box-shadow: 0 0 10px rgba(206, 2, 162, 0.86), 0 0 20px rgba(170, 4, 247, 0.64); }
        .btn-outline-danger { border: 1px solid #ff4444; color: #ff4444; }
        .btn-outline-danger:hover { background-color: #ff4444; color: #111; box-shadow: 0 0 10px #ff4444, 0 0 20px #ff4444; }

        /* ---------- Modales ---------- */
        .modal-content { border-radius: 15px; border: 1px solid rgba(255, 0, 255, 0.86); box-shadow: 0 0 20px rgba(255, 7, 152, 0.8); }
        .modal-header { border-bottom: 1px solid rgba(255, 0, 200, 0.75); }
        .modal-title { font-family: 'Ethnocentric', sans-serif; color: rgba(235, 9, 186, 0.88); }
        .modal-header.border-danger { border-bottom: 1px solid #ff4444 !important; }
        .modal-title.text-danger { text-shadow: 0 0 10px #ff4444; }

        /* ---------- Navbar toggler visible ---------- */
        .navbar-dark .navbar-toggler { border-color: rgba(255,255,255,.2); }
        .navbar-dark .navbar-toggler-icon { background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 30 30'%3e%3cpath stroke='rgba(255,255,255,0.75)' stroke-linecap='round' stroke-miterlimit='10' stroke-width='2' d='M4 7h22M4 15h22M4 23h22'/%3e%3c/svg%3e"); }

        /* ---------- Helpers ---------- */
        .stack-gap > * { margin-bottom: 1rem; }
        @media (min-width: 768px) { .stack-gap > * { margin-bottom: 1.25rem; } }
    </style>
</head>
    ...
    <nav class="navbar navbar-expand-lg navbar-dark shadow-sm py-3" style="background-color: rgb(20,20,20);">
        <div class="container-fluid">
            <a class="navbar-brand fw-bold fs-4 text-white" href="../pagina-principal/Inicio.php#inicio" id="linkLogo">
                <img src="../navbar/imagenes/logo.jpg" width="30" height="30" class="d-inline-block align-text-top" alt="Logo">
                &nbsp;Zentryx
            </a>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav me-auto">
                    <li class="nav-item"><a class="nav-link text-white" href="../pagina-principal/Inicio.php#inicio" id="linkInicio">Inicio</a></li>
                    <li class="nav-item"><a class="nav-link text-white" href="../pagina-principal/Inicio.php#juegos" id="linkJuegos">Juegos</a></li>
                    <li class="nav-item"><a class="nav-link text-white" href="../pagina-principal/ranking.php" id="linkRanking">Ranking</a></li>
                </ul>
                ...
                <ul class="navbar-nav">
                    <li class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle d-flex align-items-center text-white" id="userDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <img src="<?php echo htmlspecialchars($_SESSION['foto'] ?? '../navbar/imagenes/usuario.png'); ?>" class="user-avatar shadow-sm rounded-circle" width="36" height="36" alt="Usuario">
                        </a>
                ...
                <div class="card h-100 p-3 d-flex flex-column justify-content-between">
                    <div>
                        ...
                        <div class="d-flex flex-column align-items-center text-center">
                            ...
                            <button class="btn btn-outline-light w-100" data-bs-toggle="modal" data-bs-target="#modalImagen">Cambiar</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="stack-gap">
                    ...
                    <div class="card p-3 h-100">
                        <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-2">
                            <div class="me-sm-3">
                                ...
                                <p class="mb-1 text-secondary fw-semibold"><?php echo htmlspecialchars($datos['nom_usuario']); ?></p>
                                <p class="mb-0 text-secondary small">Modifica el nombre con el que apareces en Zentryx</p>
                            </div>
                            <div class="w-100 w-sm-auto">
                                <button class="btn btn-outline-light w-100 w-sm-auto" data-bs-toggle="modal" data-bs-target="#modalUsuario">Editar</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card p-3 h-100">
                        <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-2">
                            ...
                            <div class="w-100 w-sm-auto">
                                <button class="btn btn-outline-light w-100 w-sm-auto" data-bs-toggle="modal" data-bs-target="#modalPassword">Cambiar</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card p-3 border border-danger h-100">
                        <div class="d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-2">
                            <div class="me-sm-3">
                                ...
                                <p class="mb-0 text-secondary small">Borra permanentemente tu cuenta y todos tus datos</p>
                            </div>
                            <div class="w-100 w-sm-auto">
                                <button class="btn btn-outline-danger w-100 w-sm-auto" data-bs-toggle="modal" data-bs-target="#modalEliminar">Eliminar Cuenta</button>
                            </div>
                        </div>
                    </div>
                </div>
